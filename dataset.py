...
class SyntheticDataset(Dataset):
    def __init__(self, path, transform=None):
        ...
        if self.is_zip:
            ...
            self._all_images = [f.filename for f in self._zipfile.filelist if f.filename.endswith('.png')]
        else:
            self._all_images = [str(p.relative_to(self.path)) for p in Path(self.path).rglob('*.png')]
