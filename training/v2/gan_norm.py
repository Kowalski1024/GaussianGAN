...
class ImageGenerator(nn.Module):
    ...
    def point_cloud(self, ws, camera=None, *args, **kwargs):
        with torch.no_grad():
        ...
        return torch.stack(images, dim=0).contiguous(), torch.stack(clouds, dim=0).contiguous()
...
class Generator(nn.Module):
    ...
    def point_cloud(self, z, c, truncation_psi=1, truncation_cutoff=None, update_emas=False, **synthesis_kwargs):
        ws = self.mapping(z, None, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, update_emas=update_emas)
        return self.synthesis.point_cloud(ws, c, update_emas=update_emas)
